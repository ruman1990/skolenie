...
class Sklad:
    ...
    def __init__(self,name):
        ...
        self.conn = psycopg2.connect('dbname=skolenie user=postgres password=admin host=localhost')
        ...
        p = self.cur.fetchall()
        for x in p:
            self.produkty[x[1]] = Produkt(x[1],x[3],x[2])
        # if os.path.exists(self.file_db):
        #     with open(self.file_db,'r',encoding='utf-8') as f:
        #         self._read_file_db(f.read())

    def _zapis_do_db(self,produkt : Produkt):
        self.cur.execute('INSERT INTO produkty (nazov,pocet_kusov,cena) VALUES (%s,%s,%s)'
                         ,(produkt.nazov,produkt.pocet_kusov,produkt.cena))
        self.conn.commit()

    def _remove_z_db(self,nazov):
        self.cur.execute('DELETE from produkty where nazov=%s',(nazov,))
        self.conn.commit()

    def _change_db(self,produkt : Produkt):
        self.cur.execute('UPDATE produkty set pocet_kusov=%s , cena=%s where nazov=%s',
                         (produkt.pocet_kusov,produkt.cena,produkt.nazov))
        self.conn.commit()
    ...
    def pridanie_tovaru(self):
        ...
        if nazov in self.produkty:
            ...
            try:
                ...
                self._zapis_do_db(self.produkty[nazov])
    ...
    def naskladnenie(self):
        ...
        if nazov in self.produkty:
            ...
            self._change_db(self.produkty[nazov])
