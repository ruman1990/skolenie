...
class Sklad:
    ...
    def _zapis_do_db(self,produkt : Produkt):
        self.cur.execute('INSERT INTO produkty (nazov,pocet_kusov,cena) VALUES (?,?,?)'
                         ,(produkt.nazov,produkt.pocet_kusov,produkt.cena))
        self.conn.commit()

    def _remove_z_db(self,nazov):
        self.cur.execute('DELETE from produkty where nazov=?',(nazov,))
        self.conn.commit()

    def _change_db(self,produkt : Produkt):
        self.cur.execute('UPDATE produkty set pocet_kusov=? , cena=? where nazov=?',
